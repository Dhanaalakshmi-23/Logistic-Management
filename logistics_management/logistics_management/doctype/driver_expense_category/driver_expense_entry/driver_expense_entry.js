	...
	refresh(frm) {
		...
		if (frm.doc.docstatus === 1 && !frm.doc.expense_je) {
			frm.add_custom_button("Create Expense JE", () => {
				frappe.call({
					method: "logistics_management.logistics_management.doctype.driver_expense_entry.driver_expense_entry.create_expense_je",
		...
		if (frm.doc.docstatus === 1 && frm.doc.expense_je && !frm.doc.settlement_journal_entry) {
			frm.add_custom_button("Create Settlement JE", () => {
				frappe.call({
					method: "logistics_management.logistics_management.doctype.driver_expense_entry.driver_expense_entry.create_settlement_je",
					args: { expense_entry: frm.doc.name },
					callback: function (r) {
						if (r.message) {
